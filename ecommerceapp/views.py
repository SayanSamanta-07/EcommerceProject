...
def handlerequest(request):
    ...
    if payment_method == 'Cash on Delivery':
        if response_dict.get('RESPCODE', '') == '01':
            print('Cash on Delivery order successful')
            
            a = response_dict.get('ORDERID', '')
            b = response_dict.get('TXNAMOUNT', '')
            rid = a.replace("ShopyCart", "")
           
            print(rid)
            
            # Query the database to get the order
            ...
            for post1 in filter2:
                post1.oid = a
                post1.amountpaid = b
                post1.paymentstatus = "CASH"
                post1.save()

                print("run agede function")

        return render(request, 'paymentstatus.html', {'response': response})
    for i in form.keys():
